import { Field, invertPlayer, Game, Player, Mode, Outcome, isFull, won, getBlanks, isPlayer } from "./game";
...
describe('Game updateMode', () => {
    it('sets mode for two humans to humanmode', () => {
        const game = new Game(new Player(Field.PLAYER1), new Player(Field.PLAYER2), Mode.EASY);
        game.updateMode(Mode.HUMAN);
        expect(game.mode).toBe(Mode.HUMAN);
    }
    );
    it('sets mode for two bots to botmode', () => {
        const game = new Game(new Player(Field.PLAYER1), new Player(Field.PLAYER2), Mode.HUMAN);
...
describe('Outcome isdraw', () => {
    it('returns true if the game is a draw', () => {
        const game = new Game();
        const outcome = new Outcome([Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2]);
        expect(outcome.isDraw()).toBe(true);
    });
    it('returns false if the game is not a draw', () => {
        const game = new Game();
        const outcome = new Outcome([Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER2, Field.PLAYER1, Field.EMPTY]);
...
describe('Board isFull', () => {
    it('returns true if the board is full', () => {
        const game = new Game();
        const result = isFull([Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2]);
        expect(result).toBe(true);
    });
    it('returns false if the board is not full', () => {
        const game = new Game();
        const result = isFull([Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER2, Field.PLAYER1, Field.EMPTY]);
...
describe('Board won', () => {
    it('returns player1 if player1 has won', () => {
        const game = new Game();
        const result = won([Field.PLAYER1, Field.PLAYER1, Field.PLAYER1, Field.PLAYER2, Field.PLAYER2, Field.EMPTY, Field.EMPTY, Field.EMPTY, Field.EMPTY]);
        expect(result).toBe(Field.PLAYER1);
    });
    it('returns player2 if player2 has won', () => {
        const game = new Game();
        const result = won([Field.PLAYER2, Field.PLAYER2, Field.PLAYER2, Field.PLAYER1, Field.PLAYER1, Field.EMPTY, Field.EMPTY, Field.EMPTY, Field.EMPTY]);
        expect(result).toBe(Field.PLAYER2);
    });
    it('returns empty if no player has won', () => {
        const game = new Game();
        const result = won([Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER2, Field.PLAYER1, Field.EMPTY]);
        expect(result).toBe(Field.EMPTY);
    });
});
...
describe('Board getBlanks', () => {
    it('returns an array of empty fields', () => {
        const game = new Game();
        const result = getBlanks([Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER2, Field.PLAYER1, Field.EMPTY]);
        expect(result).toEqual([8]);
    });
    it('returns an empty array if there are no empty fields', () => {
        const game = new Game();
        const result = getBlanks([Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER1, Field.PLAYER2, Field.PLAYER2, Field.PLAYER1, Field.PLAYER1]);
